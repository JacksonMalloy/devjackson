...
export const MyThemeProvider = ({ children }) => {
  ...
  const toggle = () => {
    ...
    if (mode === Theme_Douglas) {
      let mode = themeState.mode === HSLTheme_Douglas ? Theme_Douglas : HSLTheme_Douglas
      setThemeState({ mode: mode })
    }

    if (mode === Theme_Capilano) {
      const mode = themeState.mode === Theme_Capilano ? HSLTheme_Capilano : Theme_Capilano
      setThemeState({ mode: mode })
    }

    if (mode === HSLTheme_Capilano) {
      let mode = themeState.mode === HSLTheme_Capilano ? Theme_SFU : HSLTheme_Capilano
      setThemeState({ mode: mode })
    }

    if (mode === HSLTheme_Douglas) {
      let mode = themeState.mode === HSLTheme_Douglas ? Theme_Capilano : HSLTheme_Douglas
      setThemeState({ mode: mode })
    }
  }
